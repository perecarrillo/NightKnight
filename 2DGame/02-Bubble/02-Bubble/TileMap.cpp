...
bool TileMap::collisionMoveDown(const glm::ivec2 &pos, const glm::ivec2 &size, float *posY, int offset_y, bool isOnPlatform) const
{
	int x0, x1, y;
	...
	for (int x = x0; x <= x1; x++)
	{
		if (isOnPlatform || map[y*mapSize.x + x] != 0)
		{
